...
const SearchSection: FC = () => {
  ...
  // 1. State to store the result of the click
  ...
  const handleGetNearest = () => {
    ...
    getLocation((data) => {
      if (data) {
        setUserLoc(data); // This triggers the useNearbyAirports hook
      } else {
  ...
  useEffect(() => {
    if (bestHub) {
      // Ici tu peux dispatcher vers Redux ou mettre à jour tes inputs
      console.log("Le meilleur aéroport de départ est :", bestHub.name);
      if (bestHub) {
        dispatch(fromTakeoff(bestHub));
      }
    }
  }, [bestHub]);
      ...
      <div className="w-full bg-white rounded-2xl search-shadow p-6 md:p-8 mt-4 transition-all hover:shadow-2xl ring-1 ring-black/5">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-end w-full">
          ...
          <div
            ...
            ${tripType === "ONE_WAY" ? "md:w-7/12" : "md:w-5/12"}`}
          >
            {/*// <!-- From -->*/}
          ...
          <div
            className={`
            flex flex-col sm:flex-row relative min-h-f
            w-full transition-all duration-500 ease-in-out
            ${tripType === "ONE_WAY" ? "md:w-3/12" : "md:w-5/12"}
        `}
          >
            <SearchDateTimeRange
              typeDateTime={tripType == "ROUND_TRIP" ? "DateRange" : "DateSolo"}
            />
          </div>
          {/*// <!-- Search Button -->*/}
        ...
        <div className={"inline-flex w-full p-2"}>
          <FormControlLabel
            ...
            slotProps={{
              typography: {
                className:
                  "!text-xs !text-left !font-semibold !text-slate-500 !uppercase",
              },
            }}
            control={
              <Checkbox
